...
class Individual:
    def __init__(self, id, time_weights=[], toll_weights=[]) -> None:
        self.id = id
    ...
    def get_value(self, exp_tag, num_runs):
...
def ga(num_vehicles, pop_size, num_runs, exp_tag,tournament_size=0, ga_executions=1):
    ...
    for exec in range(ga_executions):
        _p = []  
        while len(_p) < len(p):
            mother = tournament(p, tsize, exp_tag, num_runs)
            father = tournament(p, tsize, exp_tag, num_runs)
            ind1, ind2, num_individuals = crossover(mother, father, num_vehicles, num_individuals)
            ind1.mutation()
            ind2.mutation()
            _p.append(ind1)
            _p.append(ind2)
        selection(p, _p, pop_size, exp_tag, num_runs)
    return p[0] # best
...
def tournament(population, size, exp_tag, num_runs):
    best = None
    bestv = float('inf')
    for index in random.sample(range(0, len(population)), size):
        v = population[index].get_value(exp_tag, num_runs)
        if v < bestv:
...
def selection(p, _p, pop_size, exp_tag, num_runs):
    pop = sorted([ind for ind in p + _p], key=lambda ind: ind.get_value(exp_tag, num_runs))
...
print(ga(num_vehicles, pop_size, num_runs, exp_tag, tournament_size, ga_executions))